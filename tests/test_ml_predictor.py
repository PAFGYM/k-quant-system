...
class TestFeatureNames:
    def test_feature_names_length(self) -> None:
        """FEATURE_NAMES keeps the current production feature count."""
...
class TestBuildFeatures:
    def test_returns_dict_with_all_current_feature_keys(self, sample_features: dict) -> None:
        """build_features returns dict with all current production feature keys."""
        assert isinstance(sample_features, dict)
        assert set(sample_features.keys()) == set(FEATURE_NAMES)
        assert len(sample_features) == 62
